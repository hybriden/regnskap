...
/// <summary>
/// Minimal fake for IKundeReskontroRepository, only implements methods needed by FaktureringService.
/// </summary>
public class FakeKundeRepo : IKundeReskontroRepository
{
    public List<Kunde> Kunder { get; } = new();

    public Task<Kunde?> HentKundeAsync(Guid id, CancellationToken ct = default)
        => Task.FromResult(Kunder.FirstOrDefault(k => k.Id == id));

    public Task<Kunde?> HentKundeMedNummerAsync(string kundenummer, CancellationToken ct = default)
        => Task.FromResult(Kunder.FirstOrDefault(k => k.Kundenummer == kundenummer));

    public Task<bool> KundenummerEksistererAsync(string kundenummer, CancellationToken ct = default)
        => Task.FromResult(Kunder.Any(k => k.Kundenummer == kundenummer));

    public Task LeggTilKundeAsync(Kunde kunde, CancellationToken ct = default) { Kunder.Add(kunde); return Task.CompletedTask; }
    ...
    public Task<KundeFaktura?> HentFakturaMedKidAsync(string kidNummer, CancellationToken ct = default) => Task.FromResult<KundeFaktura?>(null);
    ...
    public Task<List<KundeFaktura>> HentApnePosterAsync(DateOnly? dato = null, CancellationToken ct = default) => Task.FromResult(new List<KundeFaktura>());
    public Task<List<KundeFaktura>> HentForfalteFakturaerAsync(DateOnly dato, int minimumDagerForfalt, CancellationToken ct = default) => Task.FromResult(new List<KundeFaktura>());
    ...
    public Task<(List<Kunde> Data, int TotaltAntall)> SokKunderAsync(string? query, int side, int antall, CancellationToken ct = default) => Task.FromResult((new List<Kunde>(), 0));
    public Task<(List<KundeFaktura> Data, int TotaltAntall)> SokFakturaerAsync(Guid? kundeId, KundeFakturaStatus? status, int side, int antall, CancellationToken ct = default) => Task.FromResult((new List<KundeFaktura>(), 0));
    public Task<List<KundeFaktura>> HentFakturaerForKundeAsync(Guid kundeId, DateOnly? fraDato, DateOnly? tilDato, CancellationToken ct = default) => Task.FromResult(new List<KundeFaktura>());
